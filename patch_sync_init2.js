...
function patch(filePath) {
    ...
    const initRegex = /ApiService\.syncFromCloud\(\)\.then\(success => \{\s*if \(success\) \{\s*console\.log\("Synced from cloud successfully"\);\s*applyTheme\(\);\s*\}\s*\}\);/;
    ...
    if (initRegex.test(content)) {
        content = content.replace(initRegex, newInitSync);
